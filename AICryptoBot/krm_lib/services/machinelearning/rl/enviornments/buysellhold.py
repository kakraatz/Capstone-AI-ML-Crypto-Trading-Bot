...
####### START ENVIORNMENT #########
class BuySellHold_BasicTradingEnv(gym.Env):
####### END ENVIORNMENT #########
    """ Coin Trading enviornment w/ OPEN AI gym """
    metadata = {'rendor.modes':['human']}
    
    def __init__(self, df, initial_account_balance=1000, trading_cost_rate_maker=0.0035, trading_cost_rate_taker=0.0045, window=5):
        super(BuySellHold_BasicTradingEnv, self).__init__()
        self.first_decision_step = window
        # Observation Space -> Historic window
        self.window = window
        self.space_parameters = 1
        self.space_shape = window * self.space_parameters
...
####### START ENVIORNMENT #########
class BuySellTradingEnv(gym.Env):
####### END ENVIORNMENT #########
    """ Coin Trading enviornment w/ OPEN AI gym """
    metadata = {'rendor.modes':['human']}
    
    def __init__(self, df, initial_account_balance=1000, trading_cost_rate_maker=0.0035, trading_cost_rate_taker=0.0045, window=5):
        super(BuySellTradingEnv, self).__init__()
        self.first_decision_step = window
        # Observation Space -> Historic window
        self.window = window
        ...
        # Actions of the format Long, Close
        # Actions/Decisions are Discrete - Long, Close 
        # 2 x Discrete decisions
        # 0 --> Long
        # 1 --> Close
        self.action_space = spaces.Discrete(2) # --> this was changed to 2 discrete decisions 
    ...
    def _calculate_reward(self):
        ...
        if True:
            # OPEN - Reward Structure
            if self.current_action == 0:
                #if self.current_step == 100: print("In Open")
                # 1. Next Opportunity Reward -> Reward based on next_opportunity -> standard reward
                if self.next_opportunity > 0:      
                    reward += self.next_opportunity * 1.0            # (POSITIVE REWARD) add next opportunity to reward -> next opportunity is positive the 
                else:
            ...
            # CLOSE - Reward Structure
                ...
                #if self.current_step == 100: print("In Close")
                # 1. Next Opportunity Reward -> Reward based on next_opportunity -> standard reward
                if self.next_opportunity > 0:      
                    reward += -self.next_opportunity * 0.50          # (NEGATIVE REWARD) add next opportunity to reward -> next opportunity is positive the 
                else:
    ...
    def _take_action(self, action):
        ...
        if action == 0:
            if self.open_positions < MAX_OPEN_POSITIONS:
                ...
                self.incorrect_position_calls += 1
                action_string = "Long_MAXPos"
                net_profit = self._profit_calculation(current_price, "hold_position") # When "Open" call is made when 'MAX_OPEN_POSITIONS' threshold is set this is essentially 'hold'
                #print(f"netprofit: {net_profit}")
                self.unrealized_profit = net_profit       
        # Close Positions
        if action == 1:
